...
1. **Validation server** — receives a length-agnostic `Vec<f64>` of statistical features from the Pulse SDK (314 dimensions under the v2 expansion: 176 audio + 81 motion + 57 touch — see `docs/master/BLUEPRINT-feature-pipeline-v2.md`). Runs proprietary validation models (loaded from the private `entros-validation` crate), performs cross-wallet Sybil detection via the fingerprint registry, and issues signed challenges.
